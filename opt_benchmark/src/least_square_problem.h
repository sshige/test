...
class MultivarScalarFunc
{
public:
  MultivarScalarFunc(const Eigen::VectorXd &coeff):
    coeff_(coeff)
  {}
...
typedef std::shared_ptr<MultivarScalarFunc> MultivarScalarFuncPtr;

class PolynomialFunc: public MultivarScalarFunc
{
public:
...
{
public:
  LeastSquareProblem(const MultivarScalarFuncPtr &func_ptr, unsigned int data_num=100):
    func_ptr_(func_ptr)
  {
  ...
  MultivarScalarFuncPtr func_ptr_;
  Eigen::VectorXd x_;
  Eigen::VectorXd y_;
};
